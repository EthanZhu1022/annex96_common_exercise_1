# Grouping Feature Ablation: Primary-Only Sorted

Sorted by `primary_rank` from `selected_experiment_metrics_primary_only_table.csv`. Lower rank is better.

Included rows: compact 5-feature runs plus previous larger/full grouping-feature runs (`static_extended`, `operational_profile`, `static_operational`).
