...
midterm stage. The project is not yet at the final result-analysis stage. The
conclusive outcomes so far are implementation and design outcomes: the training
pipeline is operational, multiple baselines have been implemented, and the
communication variants have been integrated into a common MAPPO framework.
...
CV-RMSE, while keeping comfort, cost, carbon, ramping, and fairness within
acceptable ranges. This objective motivates the use of a centralized critic and
structured actor communication.
...
common experimental interface for all planned methods. The remaining
uncertainty is empirical performance, not whether the methods can be trained
and evaluated in the CityLearn pipeline.

\section{Current Limitations}

The current limitation is that full training runs have not yet been completed.
Therefore, the report does not claim that one communication method is superior
to another. The final comparison will require consistent training budgets,
multiple seeds where possible, and testing on the unseen month. Another current
limitation is that K-means uses static flexibility features; future work may
replace or augment this with learned embeddings based on building trajectories.
