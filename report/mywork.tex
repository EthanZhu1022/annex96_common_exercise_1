...
the midterm stage. The project is not yet at the final result-analysis stage,
but the first 200-episode Vermont runs have been completed. The main progress so
far is that the training pipeline works, several baselines are available, and
the communication variants can be tested within the same MAPPO framework. The
current numerical results are therefore used only as preliminary reference
points.

中文：本章介绍当前已经实现的方法，以及中期阶段已经完成的工作。
项目还没有进入最终结果分析阶段，但 Vermont 场景下第一批 200 轮训练实验已经完成。
目前最主要的进展是训练流程已经可以运行，多个基线方法已经实现，多种通信变体也可以在同一个 MAPPO 框架下测试。
当前数值结果只作为阶段性参考。
...
In the communication variants, the actor first encodes each building observation
into a hidden feature. The communication layer then modifies this feature before
the action head selects the final action:
\[
...
中文：在带通信的变体中，actor 会先把每栋建筑的观测编码成隐藏特征。
...
\subsection{Baselines Without Communication}

There are two important baselines without communication. The first is standard
MAPPO: the same actor network is reused by all 25 buildings through parameter
sharing, and the centralized critic sees the full portfolio state during
training. This tests whether a basic MARL method already improves the CityLearn
task. The second is grouped MAPPO with no
communication: buildings are first divided into groups, and each group uses its
own actor, but agents do not exchange messages. This tests whether grouping
alone helps before adding communication.

中文：没有通信的基线主要有两个。
第一个是标准 MAPPO：25 栋建筑通过参数共享复用同一个 actor 网络，训练时集中式 critic 可以看到完整的建筑组合状态。
它用来测试基础的 MARL 方法本身是否已经能改善 CityLearn 任务。
第二个是无通信的分组 MAPPO：建筑先被分成不同组，每个组使用自己的 actor，但智能体之间不交换消息。
它用来测试在加入通信之前，单独分组是否有帮助。


\subsection{No Communication}

The no-communication baseline simply leaves the encoded feature unchanged:
\[
\tilde{h}_i = h_i .
\]
This makes it possible to compare grouped MAPPO with and without message
passing.
...
the agents can learn which other agents are worth paying more attention to.
...
compared through a common experimental interface. The 200-episode Vermont runs
also show that communication changes the balance between tracking, comfort, and
secondary metrics. At this stage, global CommNet looks like the most promising
communication variant, but this is only a preliminary observation.

中文：目前最重要的成果是，主要方法现在可以通过统一的实验接口运行和比较。
Vermont 场景下 200 轮训练结果也初步说明，通信机制会改变负荷跟踪、舒适性和次级指标之间的取舍。
在当前阶段，全局 CommNet 是最有前景的通信变体，但这仍然只是初步观察。


\section{Current Limitations}

The current numerical results are incomplete. They cover Vermont only, use one
seed, and are based on 200 training episodes. Therefore, this report does not
claim that one communication method is definitively better than the others. The
final comparison will need longer training, Texas RL experiments, multiple seeds
where possible, and a closer look at daily load trajectories. The CE1 schemas
...
中文：当前数值结果还不完整。
它们只覆盖 Vermont 场景，只使用一个随机种子，并且只基于 200 轮训练。
因此，本报告不会声称某一种通信方法已经明确优于其他方法。
最终比较还需要更长训练、Texas 场景下的 RL 实验、尽可能多个随机种子，以及对每日负荷轨迹的更深入分析。
当前流程使用的 CE1 schema 也没有提供电价和电网碳排放时间序列，因此如果不加入外部假设，成本和碳排放指标不能解释。
另一个限制是，K-means 当前使用的是静态设备特征；未来工作可以用基于建筑轨迹学习得到的特征来替代或补充当前特征。
