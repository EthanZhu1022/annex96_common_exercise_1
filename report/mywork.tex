...
\Bigg].
\]
Here, the NMBE and CV-RMSE terms are running portfolio-level surrogates shared
equally across all buildings, while the comfort term is computed separately for
each building. In the current code, the actual parameter values are
\[
...
中文：当前训练使用的奖励函数是自定义的
\texttt{annex96\_rewards.ce1.CE1ThreeMetricReward}。
它被设计成 CE1 三个主要目标的可训练替代形式，也就是组合层面的负荷偏差、组合层面的跟踪误差，以及每栋建筑的热舒适性。
对每个时间步中的建筑 $i$，奖励可以写成：
\[
r_i = -\Bigg[
\frac{
...
\Bigg].
\]
其中，NMBE 和 CV-RMSE 是运行过程中的组合级代理指标，并且会平均分配到所有建筑上；舒适性项则按每栋建筑分别计算。
在当前代码中，这几个参数的实际取值为：
\[
w_{\mathrm{nmbe}} = 1.0,\qquad
w_{\mathrm{cv\_rmse}} = 1.0,\qquad
...
w_{\mathrm{degree}} = 0.3.
\]
其中，binary comfort 项表示只要室内温度超出季节性舒适区间，就会产生一次超限惩罚；
degree 项则表示超出得越多，额外惩罚越大。对当前 Vermont 实验所用的 1 月和 2 月设定来说，
这个舒适温度区间是 \(20^\circ\mathrm{C}\) 到 \(24^\circ\mathrm{C}\)。
