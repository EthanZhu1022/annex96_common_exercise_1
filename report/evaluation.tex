...
中文：本章基于 \texttt{final2}/\texttt{final3} 实验结果，报告当前的
...
hours. In this run, the worst building reaches 454 exceedance hours, compared
with a mean of about 174.9 hours, which is roughly 2.6 times the average.
...
Three the communication conclusions can now be stated more clearly. First,
the strongest overall communication variant is now PowerNet Global rather than
\texttt{comm\_v2}. A simple interpretation is that the PowerNet-style block
preserves local information while adding structured peer information; this can
be understood as a form of information filtering, so useful features are less
likely to be overwritten during communication
\cite{powernet2020}. Second, the weighted communication family shows that the
choice of communication weights matters a lot. Information from other groups can
act as noise, but it can also contain important portfolio-level context, so
changing the same-group and other-group weights changes the final result
substantially. Third, TarMAC can be understood as a stronger extension of
weighted communication: instead of using fixed coefficients, attention allows
each agent to learn which other buildings matter more in the current state
\cite{tarmac2019}. In the current implementation, however, TarMAC still uses
CommNet-style mean aggregation as its communication base rather than the
PowerNet-style selective message construction \cite{commnet2016}. This is one
reason why combining TarMAC with PowerNet is a promising next step. Finally,
DIAL was originally proposed for DQN-style discrete-action settings, where
differentiable messages help agents learn from the consequences of their own
communicated choices; during training, continuous messages are used and then
discretized through a sigmoid-like mechanism at execution time \cite{dial2016}.
In the present MAPPO-based continuous-control setting, this adaptation performs
poorly, which suggests that the original DIAL idea does not transfer directly.
...
baseline. In the current setting, the secondary metrics are not explicitly
included in the reward function, so these differences appear as natural
outcomes of the learned communication structure. PowerNet Global is not the best
in comfort, but it gives a more balanced outcome across the primary objective
and some secondary metrics, so it rises to first overall even though it is not
the best on comfort alone.
...
RBC, and compared with independently trained PPO and SAC they better capture
portfolio-level coordination; in particular, they avoid the larger comfort cost
seen in SAC while also going beyond the lack of explicit coordination in
independent PPO. Second,
communication matters, but the best communication is not fixed in advance. In
the current result set, PowerNet Global is the strongest all-round variant,
which suggests that applying more selective filtering or cropping to shared
information before fusion is effective. Third, the weighted communication family shows that
message weights themselves matter: information from other groups can either act
as noise or provide useful portfolio-level context. Fourth, TarMAC and DIAL
show that not every trainable communication mechanism transfers equally well to
this setting. Attention-based weighting remains promising, especially if later
combined with more selective PowerNet-style information filtering, whereas the
current MAPPO adaptation of DIAL performs poorly.
...
The main remaining limits are now less about unfinished runs and more about what
has not yet been fully explored in the communication design itself. The current
comparison still uses one seed and one climate, but the larger methodological
gap is that most communication modules are still tested as separate designs.
Future work should therefore focus on upgrading and combining them, especially
by fusing attention-based weighting with more selective PowerNet-style
information filtering, while also testing stronger PPO/critic variants and
cross-family comparisons with methods such as MASAC or value-based MARL. Cost
and carbon metrics also remain unavailable because the CE1 schemas used here do
not supply pricing or carbon-intensity files.
