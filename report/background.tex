...
This project uses the Annex 96 Common Exercise 1 neighborhood datasets in
...
中文：本项目使用 CityLearn 中的 Annex 96 Common Exercise 1（CE1）社区数据集。
每个 CE1 schema 定义了 25 栋独栋住宅、一个天气文件、一个区域目标负荷文件，以及每栋住宅各自的能耗仿真文件。
其中区域目标负荷文件给出了整个建筑组合需要跟踪的参考负荷；
每栋住宅自己的文件则提供本地需求、室内温度、太阳能发电等时间序列观测。
对本项目最重要的一点是，CE1 是建筑组合负荷跟踪任务，而不是物理电网拓扑控制任务。
...
中文：当前使用的 CE1 schema 没有提供住宅之间的馈线拓扑、楼层或空间分布、地理坐标，也没有提供建筑之间的邻接关系。
因此，本项目当前的分组方法不能基于物理距离或电气拓扑。
实际使用的是 CityLearn 初始化环境后可以读取到的灵活性相关设备特征，主要是电池容量和 HVAC 名义总功率。
这与实验中的 K-means 分组实现一致。
...
中文：另一个需要说明的数据限制是电价和碳排放强度。
CE1 schema 中虽然保留了 electricity pricing 和 carbon intensity 这些观测名称，但本项目使用的 VT 和 TX CE1 建筑条目中，pricing 文件和 carbon-intensity 文件都设置为 null。
CityLearn 在这种情况下会用全 0 序列填充这些数据。
因此，成本和碳排放指标在 CityLearn 中仍然有计算公式，但在当前实验中会表现为 0 或 null，不能作为有意义的性能结果解释。
...
中文：规则控制（RBC）是 CityLearn 中重要的非学习基线。
RBC 控制器使用固定规则，通常根据一天中的小时来决定储能充放电，以及供暖或制冷设备的可用功率。
它简单、透明、运行成本低，但不能根据 CE1 的目标负荷曲线或建筑之间的相互影响自适应调整策略。
因此，本项目把 RBC 作为一个实际参考点：一个有效的 MARL 方法最终应该不只是重复固定时段规则。
...
on-policy 和 off-policy 的主要区别在于数据怎么使用。
PPO 和 MAPPO 这类 on-policy 方法使用当前策略采样得到的新轨迹进行更新，训练通常更稳定、更容易控制，但样本利用率较低。
SAC 这类 off-policy 方法可以把过去的经验放进 replay buffer，并反复用于多次更新，因此样本利用率和探索能力通常更强，但也更容易受到旧数据和当前策略分布不一致的影响。
...
\begin{itemize}
  ...
  tracking. In the current CE1 data, cost and carbon are not interpreted because
  the pricing and carbon-intensity files are absent;
  中文：很多 MARL 通信研究使用抽象博弈环境，而不是舒适性、爬坡、峰值负荷和负荷跟踪等建筑能源指标。需要注意的是，当前 CE1 数据没有提供电价和碳排放强度文件，因此本实验不解释成本和碳排放结果。
