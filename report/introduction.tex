...
第二，比较不同类型的 MARL，例如 MAPPO、MASAC 或基于价值函数的方法，判断哪类学习方式更适合这个任务。
第三，在 MAPPO 框架下比较有通信和无通信的版本，并进一步比较不同通信结构。
...
中文：当前阶段选择 MAPPO 作为主要实验主干，是因为它是 on-policy 的集中训练、分散执行方法，PPO 类更新通常比较稳定，也更容易调试 \cite{ppo2017,mappo2022}。这不是说 MAPPO 一定是最终最优算法，而是因为它适合作为第一阶段的稳定起点，用来调通 CityLearn 环境、分组 actor、共享 critic 和通信模块。等这条流程稳定后，后续会继续和 SAC/MASAC、DQN 类价值函数方法等方向做对比。
...
中文：因此，通信部分是在当前 MAPPO 主干上的优化和创新。它研究的是建筑智能体之间是否应该共享信息；如果要共享，是简单均值消息、图结构邻居通信、注意力通信，还是组内/组间加权通信更有效。
