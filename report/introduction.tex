...
information passing, attention-based communication, or weighted
same-group/other-group communication is more effective.

中文：因此，通信部分是在当前 MAPPO 主干上的优化和创新。
它研究的是建筑智能体之间是否应该共享信息；
如果要共享，是简单均值消息、PowerNet 风格结构化信息传递、注意力通信，还是组内/组间加权通信更有效。
...
\begin{itemize}
  ...
  \item 多种可微通信模块，包括 CommNet 风格均值池化、PowerNet 风格结构化信息传递、TarMAC 风格注意力、DIAL 启发的全局通信，以及组内/组间加权通信；
  \item 日志、模型保存、测试和 Annex 96 指标导出工具。
\end{itemize}

The project now also has a first set of Vermont 200-episode runs and a separate
RBC baseline comparison. These results are useful as partial evidence: they show
that the MAPPO and communication pipelines execute end-to-end and reveal early
trade-offs between tracking, comfort, peak demand, and fairness. They are not
treated as final results because the training budget is limited, Texas has not
yet been completed for the RL methods, and additional seeds are still needed for
stronger conclusions. Therefore, this report presents the literature review,
research gap, method, implementation progress, preliminary results, and the
remaining evaluation plan.

中文：目前项目也已经有第一批 Vermont 场景下 200 轮训练的结果，以及单独的 RBC 基线对比。
这些结果可以作为阶段性参考，说明 MAPPO 和通信方法的流程能够完整跑通，
也初步展示了负荷跟踪、舒适性、峰值负荷和公平性之间的取舍。
但这些还不是最终结果，因为训练轮数有限，Texas 场景下的 RL 实验还没完成，也还需要更多随机种子来支持更可靠的结论。
