...
class CityLearnEnv(Environment, Env):
    ...
    **kwargs : dict
    ...
    @property
    def district_net(self) -> float:
        """Current district net electricity consumption (only positive imports from grid)."""
        
        return self.__district_net_current
    
    @property
    def district_tracking_error(self) -> float:
        """Current district tracking error (district_target - district_net)."""
        
        return self.__district_tracking_error_current
    ...
    def observation_names(self) -> List[List[str]]:
        ...
        if self.central_agent:
            ...
            observation_names.append('district_load')
    ...
    def get_default_shared_observations() -> List[str]:
        ...
        return [
            ...
            'district_load_target', 'district_net', 'district_tracking_error'
        ]
    ...
    def reset(self, seed: int = None, options: Mapping[str, Any] = None) -> Tuple[List[List[float]], dict]:
        ...
        self.__district_net_current = 0.0
        self.__district_tracking_error_current = 0.0
        self.update_variables()
    ...
    def update_variables(self):
        ...
        # district net (solo importazioni positive dalla rete)
        district_net = sum([max(0.0, b.net_electricity_consumption[self.time_step]) for b in self.buildings])
        
        # district tracking error
        district_target = self.buildings[0].district_load_target.district_load_target[self.time_step]
        district_tracking_error = district_target - district_net
        
        # Salva come attributi per le osservazioni
        self.__district_net_current = district_net
        self.__district_tracking_error_current = district_tracking_error
