...
REQUIRED_CORES=$((3 * THREADS_PER_JOB))
if ((${#CPU_IDS[@]} < REQUIRED_CORES)); then
  echo "Need at least $REQUIRED_CORES allowed CPU cores, but only ${#CPU_IDS[@]} are available." >&2
  exit 2
fi
...
run_seed() {
  ...
  for ((offset = 0; offset < THREADS_PER_JOB; offset++)); do
    cpu_spec+="${cpu_spec:+,}${CPU_IDS[$((slot * THREADS_PER_JOB + offset))]}"
  done
