...
RUN_SCRIPT="$REPO_DIR/scripts/run_tarmac_3fA_vt_degree_hours_3seeds_parallel.sh"

screen -dmS "$SESSION_NAME" bash -lc \
  "set -o pipefail; cd '$REPO_DIR' && bash '$RUN_SCRIPT' 2>&1 | tee '$SCREEN_LOG'"

echo "Started detached screen session: $SESSION_NAME"
echo "All three VT seeds run concurrently; each seed is pinned to two CPU cores."
