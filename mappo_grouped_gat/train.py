"""
Grouped MAPPO with K-means-derived GAT communication and grouped actor heads.

Actor path:
  obs(group_k) -> encoder_k -> graph-masked GAT communication -> action_head_k
"""
...
from mappo_grouped_gat.cluster import run_clustering  # noqa: E402
from mappo_grouped_gat.env import CityLearnMAPPOEnv  # noqa: E402
from mappo_grouped_gat.gat_comm import GroupSimilarityGATCommunicationModule  # noqa: E402
...
class Config:
    ...
    wandb_name: str = "mappo-grouped-gat"
    save_dir: str = "results/mappo_grouped_gat"
    backup_dir: Optional[str] = None

    comm_method: str = "gat"
    comm_rounds: int = 1
    comm_use_residual: bool = False
    comm_dropout: float = 0.0
    comm_scope: str = "kmeans_similarity_graph"
    comm_num_heads: int = 4
    comm_inter_group_top_k: int = 1
    comm_self_loops: bool = True
    comm_negative_slope: float = 0.2
    comm_activation: str = "elu"


def build_kmeans_similarity_graph(
    group_indices: List[np.ndarray],
    centers_orig: Optional[np.ndarray],
    inter_group_top_k: int = 1,
    self_loops: bool = True,
) -> Tuple[np.ndarray, Dict[str, Any]]:
    """Construct same-group full edges plus sparse nearest-group bridge edges.

    The graph is directed for attention masking, but bridge selection is made
    symmetric so information can flow both ways between selected groups.
    """
    group_indices = [np.asarray(idx, dtype=int) for idx in group_indices]
    n_groups = len(group_indices)
    n_agents = int(sum(len(idx) for idx in group_indices))
    adjacency = np.zeros((n_agents, n_agents), dtype=bool)

    for idx in group_indices:
        adjacency[np.ix_(idx, idx)] = True

    selected_group_edges: List[Dict[str, int]] = []
    inter_group_top_k = max(int(inter_group_top_k), 0)
    if inter_group_top_k > 0 and n_groups > 1:
        if centers_orig is not None:
            centers = np.asarray(centers_orig, dtype=np.float64)
            if centers.shape[0] != n_groups:
                raise ValueError(
                    f"Expected {n_groups} cluster centers, got shape {tuple(centers.shape)}."
                )
            scale = centers.std(axis=0)
            scale[scale < 1e-9] = 1.0
            centers = (centers - centers.mean(axis=0)) / scale
            distances = np.linalg.norm(
                centers[:, None, :] - centers[None, :, :],
                axis=-1,
            )
        else:
            group_ids = np.arange(n_groups, dtype=np.float64)
            distances = np.abs(group_ids[:, None] - group_ids[None, :])

        for src_group in range(n_groups):
            order = [
                int(g)
                for g in np.argsort(distances[src_group]).tolist()
                if int(g) != src_group
            ]
            for dst_group in order[:inter_group_top_k]:
                src_idx = group_indices[src_group]
                dst_idx = group_indices[dst_group]
                adjacency[np.ix_(src_idx, dst_idx)] = True
                adjacency[np.ix_(dst_idx, src_idx)] = True
                selected_group_edges.append(
                    {
                        "source_group": src_group,
                        "target_group": dst_group,
                        "distance_rank": len(
                            [e for e in selected_group_edges if e["source_group"] == src_group]
                        )
                        + 1,
                    }
                )
    ...
    row_has_neighbor = adjacency.any(axis=1)
    if not np.all(row_has_neighbor):
        missing = np.where(~row_has_neighbor)[0].tolist()
        raise ValueError(f"GAT adjacency has nodes without neighbors: {missing}")

    same_group = np.zeros_like(adjacency)
    for idx in group_indices:
        same_group[np.ix_(idx, idx)] = True
    if not self_loops:
        np.fill_diagonal(same_group, False)

    n_edges = int(adjacency.sum())
    intra_edges = int((adjacency & same_group).sum())
    inter_edges = int(n_edges - intra_edges)
    graph_summary: Dict[str, Any] = {
        "graph_type": "kmeans_similarity_graph",
        "construction": (
            "same-cluster fully connected edges plus sparse bidirectional "
            "nearest-cluster bridge edges"
        ),
        "n_agents": n_agents,
        "n_groups": n_groups,
        "group_sizes": [int(len(idx)) for idx in group_indices],
        "self_loops": bool(self_loops),
        "inter_group_top_k": int(inter_group_top_k),
        "n_directed_edges": n_edges,
        "n_intra_group_edges": intra_edges,
        "n_inter_group_edges": inter_edges,
        "edge_density": float(n_edges / max(n_agents * n_agents, 1)),
        "selected_group_edges": selected_group_edges,
        "paper_reference": "Velickovic et al., Graph Attention Networks, ICLR 2018",
        "ce1_topology_note": (
            "CE1 does not provide feeder/geographic adjacency; this graph is "
            "constructed from K-means building similarity."
        ),
    }
    return adjacency, graph_summary


def save_graph_artifacts(adjacency: np.ndarray, graph_summary: Dict[str, Any], save_dir: Path) -> None:
    """Save GAT graph artifacts next to cluster and metric artifacts."""
    save_dir = Path(save_dir)
    adjacency_path = save_dir / "gat_graph_adjacency.csv"
    with adjacency_path.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["source_building"] + [f"target_{i}" for i in range(adjacency.shape[1])])
        for i, row in enumerate(adjacency.astype(int).tolist()):
            writer.writerow([i] + row)

    summary_path = save_dir / "gat_graph_summary.json"
    summary_path.write_text(json.dumps(graph_summary, indent=2))
    print(f"  [graph] adjacency CSV -> {adjacency_path}")
    print(f"  [graph] summary JSON  -> {summary_path}")
...
def _build_grouped_components(
    ...
    if graph_adjacency is None:
        graph_adjacency, _ = build_kmeans_similarity_graph(
            group_indices=group_indices,
            centers_orig=None,
            inter_group_top_k=cfg.comm_inter_group_top_k,
            self_loops=cfg.comm_self_loops,
        )

    global_comm = GroupSimilarityGATCommunicationModule(
        hidden_dim=cfg.hidden_size,
        adjacency_mask=torch.as_tensor(graph_adjacency, dtype=torch.bool, device=device),
        num_heads=cfg.comm_num_heads,
        comm_rounds=cfg.comm_rounds,
        use_residual=cfg.comm_use_residual,
        dropout=cfg.comm_dropout,
        negative_slope=cfg.comm_negative_slope,
        activation=cfg.comm_activation,
    ).to(device)
...
def evaluate_on_test(
    ...
    print("\n" + "=" * 65)
    print(
        f"TEST EVALUATION (grouped GAT) | {cfg.climate} | {month_name} "
...
def train(cfg: Config) -> None:
    ...
    K = int(group_assignments.max()) + 1
    group_indices_for_graph = [np.where(group_assignments == k)[0] for k in range(K)]
    graph_adjacency, graph_summary = build_kmeans_similarity_graph(
        group_indices=group_indices_for_graph,
        centers_orig=cluster_result.get("centers_orig"),
        inter_group_top_k=cfg.comm_inter_group_top_k,
        self_loops=cfg.comm_self_loops,
    )
    save_graph_artifacts(graph_adjacency, graph_summary, cluster_dir)
    ...
    print("\nGrouped MAPPO GAT components built:")
    ...
    print(
        f"  graph_edges={graph_summary['n_directed_edges']} "
        f"intra={graph_summary['n_intra_group_edges']} "
        f"inter={graph_summary['n_inter_group_edges']} "
        f"density={graph_summary['edge_density']:.3f}"
    )
        ...
        wandb.log(
            {
                ...
                "train/comm/num_heads": cfg.comm_num_heads,
                "train/comm/inter_group_top_k": cfg.comm_inter_group_top_k,
                "train/comm/self_loops": int(cfg.comm_self_loops),
                "train/graph/n_directed_edges": graph_summary["n_directed_edges"],
                "train/graph/n_intra_group_edges": graph_summary["n_intra_group_edges"],
                "train/graph/n_inter_group_edges": graph_summary["n_inter_group_edges"],
                "train/graph/edge_density": graph_summary["edge_density"],
            },
            step=0,
        )
    ...
    print("\nGrouped MAPPO GAT training complete.")
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(
        description="Grouped MAPPO with K-means-derived GAT communication and grouped actor heads."
    )
    ...
    parser.add_argument("--wandb_name", default="mappo-grouped-gat")
    parser.add_argument("--save_dir", default="results/mappo_grouped_gat")
    ...
    parser.add_argument("--comm_num_heads", type=int, default=4)
    parser.add_argument("--comm_inter_group_top_k", type=int, default=1)
    parser.add_argument("--no_comm_self_loops", action="store_true", default=False)
    parser.add_argument("--comm_negative_slope", type=float, default=0.2)
    parser.add_argument("--comm_activation", default="elu", choices=["elu", "tanh", "none", "identity"])
    ...
    return Config(
        ...
        comm_num_heads=args.comm_num_heads,
        comm_inter_group_top_k=args.comm_inter_group_top_k,
        comm_self_loops=not args.no_comm_self_loops,
        comm_negative_slope=args.comm_negative_slope,
        comm_activation=args.comm_activation,
    )
