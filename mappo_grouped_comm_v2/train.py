"""
Experimental grouped MAPPO with global communication and grouped actor heads.

Compared with mappo_grouped:
  - encoders stay group-specific
  - action heads stay group-specific
  - communication runs globally across all agents before action heads

Actor path:
  obs(group_k) -> encoder_k -> global communication over all agents -> action_head_k
"""
...
from mappo_grouped_comm_v2.cluster import run_clustering  # noqa: E402
from mappo_grouped_comm_v2.env import CityLearnMAPPOEnv  # noqa: E402
...
class Config:
    ...
    wandb_name: str = "mappo-grouped-comm-v2"
    save_dir: str = "results/mappo_grouped_comm_v2"
    backup_dir: Optional[str] = None

    comm_method: str = "commnet"
...
def _compute_advantages(trainer: R_MAPPO, buffer: GroupedSharedReplayBuffer) -> np.ndarray:
...
def _joint_actor_generator(
    ...
    for indices in sampler:
        ...
        for buffer, advantages in zip(buffers, advantages_by_group):
            ...
            obs = buffer.obs[:-1].reshape(batch_size, num_agents, *buffer.obs.shape[3:])
            rnn_states = buffer.rnn_states[:-1].reshape(
                batch_size, num_agents, *buffer.rnn_states.shape[3:]
            )
            actions = buffer.actions.reshape(batch_size, num_agents, buffer.actions.shape[-1])
            masks = buffer.masks[:-1].reshape(batch_size, num_agents, 1)
            active_masks = buffer.active_masks[:-1].reshape(batch_size, num_agents, 1)
            ...
            batch_dict: Dict[str, np.ndarray] = {
                ...
                "rnn_states": rnn_states[indices].reshape(-1, *rnn_states.shape[2:]),
                ...
                "old_log_probs": old_log_probs[indices].reshape(-1, old_log_probs.shape[-1]),
...
def _build_grouped_components(
    ...
    global_comm = build_communication_module(
        ...
        comm_share_within_group_only=False,
    ).to(device)
...
def _train_global_actor(
    cfg: Config,
    controller: GlobalCommActorController,
    actor_optimizer: torch.optim.Optimizer,
    trainers: Sequence[R_MAPPO],
    buffers: Sequence[GroupedSharedReplayBuffer],
...
def save_checkpoint(
    policies: Sequence[R_MAPPOPolicy],
    controller: GlobalCommActorController,
    actor_optimizer: torch.optim.Optimizer,
    group_assignments: np.ndarray,
    ...
    ckpt = {
        ...
        "global_actor_state_dict": controller.state_dict(),
        "global_actor_optimizer": actor_optimizer.state_dict(),
...
def load_checkpoint(
    ...
    controller.load_state_dict(ckpt["global_actor_state_dict"])
    if "global_actor_optimizer" in ckpt:
        actor_optimizer.load_state_dict(ckpt["global_actor_optimizer"])
    episode = ckpt.get("episode", 0)
    print(f"  [ckpt] loaded checkpoint from {ckpt_path} (episode {episode})")
    return episode
...
def evaluate_on_test(
    ...
    print("\n" + "=" * 65)
    print(
        f"TEST EVALUATION (global comm v2) | {cfg.climate} | {month_name} "
...
def train(cfg: Config) -> None:
    ...
    print(f"\nGrouped MAPPO v2 components built:")
    ...
    print("\nGrouped MAPPO v2 training complete.")
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(
        description="Experimental grouped MAPPO with global communication and grouped actor heads."
    )
    ...
    parser.add_argument("--wandb_name", default="mappo-grouped-comm-v2")
    parser.add_argument("--save_dir", default="results/mappo_grouped_comm_v2")
    ...
    parser.add_argument("--comm_method", default="commnet", choices=["none", "commnet"])
