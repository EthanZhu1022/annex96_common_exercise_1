"""
Global communication actor controller.

This module owns the actor-side execution path for the v2 experiment:

1. Each group uses its own actor encoder.
2. Encoded features from all agents are assembled into one global tensor.
3. A single communication module runs over all agents.
4. The communicated features are split back to each group.
...
class GlobalCommActorController(nn.Module):
    """Run global communication with grouped encoders and grouped action heads."""
    ...
    def act(
        ...
        """Run one rollout / eval step with global communication."""
        obs = check(obs).to(**self.tpdv)
        rnn_states = check(rnn_states).to(**self.tpdv)
        masks = check(masks).to(**self.tpdv)
