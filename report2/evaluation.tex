...
\begin{zhtranslation}
表~\ref{tab:router-progression}列出了测试过的 Soft Router 版本，
图~\ref{fig:router-progression}展示了主要开发阶段的性能变化。除最终 stable-head 行使用
三个 router-training seeds 的均值和样本标准差外，其余中间版本均只使用 seed 42。这是
因为这些版本的结果已经较差，而重复运行多 seed 实验需要很高的时间和计算成本，因此只对
最终的 stable 版本进行了多 seed 验证。

Legacy end-to-end 版本从一开始就同时训练 router 和 actors。它的 CV-RMSE 为47.60\%，
但绝对 NMBE 为4.31\%，每栋建筑每天的平均 degree-hours 为10.02，舒适超限为24.64\%。
它能够学出基本合理的负荷曲线，但负荷偏差和热舒适度较差。

Two-stage 版本先载入训练好的 3fA experts，再训练 router，因此 bias 立即降低。Freeze200
版本的 CV-RMSE 为48.26\%，绝对 NMBE 为1.35\%；调整温度和 prior 后，prior0.7 版本将
CV-RMSE 改善到46.54\%。移除七个物理 capacity 输入后，绝对 NMBE 降至表中最低的0.57\%，
每栋建筑每天的平均 degree-hours 也降至5.53。这说明，载入有效 experts 比给 router 增加
大量额外输入更重要。

Three-stage 实验用于比较应该保留 expert 的哪些部分。Shared-encoder 版本是表现最差的
版本，CV-RMSE 为52.44\%，绝对 NMBE 为7.48\%，舒适超限为31.35\%。相比之下，保留每个
完整 expert 分支后，CV-RMSE 降至45.45\%。这说明 expert 的 encoder、communication path
和 output head 应保持为一个整体；重新建立共享的内部表示会破坏 actors 已经学到的有效行为。

Stable 版本限制了 router 训练完成后还能改变的部分。Seed-42 router-only 实验的 CV-RMSE
为45.99\%，每栋建筑每天的平均 degree-hours 为4.16。五次可用 router-only 实验的
CV-RMSE 为 $49.83\pm3.34$\%，绝对 NMBE 为 $1.19\pm0.60$\%，degree-hours 为
$4.55\pm1.25$，舒适超限为 $18.73\pm2.43$\%。Router 稳定后只允许 action heads 小幅
调整，得到最终最强结果：三个 router-training seeds 的 CV-RMSE 为 $45.12\pm0.35$\%，
绝对 NMBE 为 $1.35\pm0.33$\%，每栋建筑每天的平均 degree-hours 为 $3.78\pm0.44$，
舒适超限为 $17.30\pm1.33$\%。
\end{zhtranslation}
...
different seeds. The final method should therefore be described as
a useful and stable improvement built on strong fixed experts, not as a method
that removes the need for good initial grouping.

\begin{zhtranslation}
总体而言，stable-head Soft Router 是目前效果最好、也最稳定的 routing 版本。它的平均
CV-RMSE 和 degree-hours 优于固定 3fA 五次实验的 $49.86\pm3.65$\% 和
$4.64\pm1.27$。三个 stable-head 实验都从同一组 seed-42 预训练 actors 开始，只改变后续
router 训练使用的随机种子。因此，这些结果只能说明 router 可以较稳定地训练，不能说明
从头训练整个模型时也能在不同 seeds 下保持同样稳定。因此，最终方法stable-head Soft Router是建立在强固定 experts 上的
有效且稳定的提升方法，而不是能够取消良好初始分组的方法。
\end{zhtranslation}
