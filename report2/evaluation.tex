...
grouping and the linear-Hybrid backbone. The 3fA and 5F rows are the means over
all five available runs (seeds 0, 1, 2, 3, and 42), while 4F retains its only
available result, seed 42. The 3fA mean is best on all four reported metrics.
The 5F mean has a similar CV-RMSE but larger bias and comfort errors, while the
single 4F result is worse than the 3fA mean on every metric. Since 4F currently
has only one seed, its comparison should be treated as preliminary. Overall,
the results support the low-redundancy hypothesis: more standardized columns
can change the geometry of grouping even when each column appears individually
relevant.
...
\end{tabular}
\end{center}
\caption{Feature-count ablation with agglomerative grouping. The 3fA and 5F
rows are five-seed means; the 4F row uses its only available run, seed 42.}
...
\end{center}
\caption{Feature-count and three-feature combination ablations from recorded
runs. In the feature-count panel, 3fA and 5F use five seeds and 4F uses seed 42.}
\label{fig:feature-ablation}
\end{figure}

\begin{zhtranslation}
表~\ref{tab:feature-count}在相同凝聚聚类、 linear Hybrid和多seed均值 下比较 3fA、4F、5F。3fA 的均值在四项指标上都最好。5F 的平均 CV-RMSE 与 3fA 接近，但
bias 和舒适度指标更差；4F 的均值结果也在所有指标上差于 3fA 均值。总体而言，这支持低冗余假设：即使每个特征单独
看来都相关，增加更多标准化列仍可能改变聚类结果并降低性能。
\end{zhtranslation}
