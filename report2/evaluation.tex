...
\begin{zhtranslation}
上述图像展示固定 3fA 和 stable Soft Router 中选定最佳实验的详细测试结果。固定 3fA
使用 seed 42，因为它的 checkpoint
...
GMM、凝聚和 balanced spectral 会形成不同的精度。第三，分组特征并不是越多越好。选择
描述建筑不同方面、重复信息较少的少量特征，通常比加入许多相似特征更有效。3fA 的低
bias、清楚的物理含义和良好的跨气候表现支持了这一结论。第四，Soft Router 实现了
Introduction 中提出的主要创新点。它让每栋建筑根据当前状态选择不同的 actor，而不是
始终使用固定分组分配的同一个 actor。因此，它能够提高性能和 seed 稳定性，并减少模型
对固定分组的依赖。

不过，Soft Router 还不能完全消除这种依赖。End-to-end 和 shared-representation 版本
从头训练时表现较差，只有载入效果较好的预训练 experts 后才能得到稳定结果。因此，
Soft Router 是一种建立在良好固定分组之上的提升方法：它减少了每栋建筑始终绑定同一个
actor 的限制，但初始 experts 的质量仍然会影响最终结果。
\end{zhtranslation}
