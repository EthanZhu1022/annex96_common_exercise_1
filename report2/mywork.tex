...
training descriptions also call it an expert. At every time step,
the observation of each building is first converted into a compact internal
representation and combined with the information received from the other
buildings through TarMAC. In the capacity-aware version, the router also receives
seven easy-to-interpret building variables: normalized battery capacity,
normalized HVAC rating, battery state of charge, remaining charging space,
currently available discharge energy, current non-shiftable load, and current
heating demand. The first two describe the building's equipment, while the other
five describe its current operating condition. The remaining charging space is
...
在每个时间步，实验先把每栋建筑的
观测转换成紧凑的内部表示，并通过 TarMAC 与其他建筑的信息结合。在 capacity-aware 版本中，
router 还会接收七个容易理解的建筑变量：归一化电池容量、归一化 HVAC 额定功率、电池 SOC、
剩余可充电空间、当前可放电电量、当前不可转移负荷和当前供暖需求。前两个变量描述建筑的
设备能力，后五个变量描述当前运行情况。剩余可充电空间由电池容量和未使用的电量比例计算，
当前可放电电量则由电池容量和当前 SOC 计算。
