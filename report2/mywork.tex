...
features, and then uses the connection pattern to form groups. A fixed number of
places is created in every group. The
...
Every selected column is standardized before clustering. The feature study fixes
agglomerative clustering as the grouping method and consists of two stages.

The first stage is the feature-count ablation, which compares a three-feature
candidate A with two larger sets:
\begin{itemize}
...
\end{itemize}
The second stage is the feature-combination ablation. It holds the count at three
...
This design separates the effect of feature count from the effect of which
physical information is included.
...
标准化。特征实验固定使用凝聚聚类作为分组方法，并分为两个阶段进行。

第一阶段是特征数量消融，将三特征组合 A 与两个更大的特征集合比较：三特征组合 A 包含
电池容量、平均供暖需求和平均不可
转移负荷；4F 在组合 A 上增加 HVAC 总额定功率；5F 再增加平均舒适下界超限。

第二阶段是特征组合消融。实验把特征数量固定为三个，通过 mean/max 替换、用 HVAC 替代
热需求、增加舒适度或温度统计以及移除电池容量，比较不同物理信息的影响。这个设计把
“特征数量”的影响和“具体选择哪些物理信息”的影响分开研究。
