...
Each trained group actor is treated as one candidate actor, also called an
expert. To explain how the router chooses among them, first consider how one
expert produces an action. At every time step, a building provides an observation
...
building action. Let $\pi_{\phi_k}(a_{it}\mid s_t)$ denote the action distribution
of expert $k$, with expert parameters $\phi_k$. The complete routed policy is the
mixture
\begin{equation}
...
\end{equation}
This equation is the key reason why the router can still be trained when all
experts are frozen. Freezing an expert fixes $\phi_k$ and its action distribution,
but it does not fix the complete policy: changing $\theta$ changes the mixture
weights $g_{itk}$ and therefore changes the probability of the executed action.
...
them. The original-group prior and gradual increase of $\alpha$ prevent abrupt
policy changes, PPO clipping limits each probability update, and the balance term
prevents all buildings from collapsing onto one expert. In the stable full-expert
experiment, episodes 1--500 update the router while freezing all experts;
...
每个已经训练好的分组 actor 都被看作一个候选 actor，也称为 expert。要说明 router 怎样在
这些 expert 之间选择，可以先看一个 expert 怎样产生动作。在每个时间步，每栋建筑提供观测
...
训练时根据 $g_{it}$ 选择 expert，再由该 expert 生成建筑动作。令 expert $k$ 的动作分布为
...
\end{equation}
这个公式解释了为什么冻结全部 expert 后仍然能训练 router。冻结 expert 只是固定 $\phi_k$ 和
各自的动作分布，并没有固定完整 policy；router 参数 $\theta$ 改变时，混合权重 $g_{itk}$
仍会改变，因此实际动作的总概率也会改变。
...
参考点，参数量更小的 router 只需要学习如何组合这些已有能力。原固定分组 prior 和逐步增加
$\alpha$ 避免策略突然改变，PPO clipping 限制每次概率更新幅度，平衡项避免选择塌缩到单一
expert。在 stable full-expert 实验中，第1--500个 episode 冻结全部 expert、只更新 router；
