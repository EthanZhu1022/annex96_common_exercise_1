...
Each trained group actor is treated as one candidate actor; the following
training descriptions also call it an expert. At every time step, each building
provides an observation. Here, an observation means the information that the
controller can see at that moment, such as battery state of charge, current
non-shiftable load, heating demand, indoor temperature, and time information. It
does not mean the complete building dataset.

The first part of the actor network converts these observation values into a
learned internal representation. This representation is not a new physical
measurement; it is a fixed-length set of numbers in which the network summarizes
the parts of the observation that it considers useful for control. TarMAC then
combines this summary with information from the other buildings. The standard
Soft Router selects an actor mainly from this communicated summary.

However, this communicated summary may not clearly retain some physical
information that is directly useful for actor selection, such as battery
capability and current load.

The capacity-aware version is an enhanced Soft Router that gives the selector
additional physical information instead of asking it to infer everything from
the summary above. It receives seven extra variables: normalized battery
capacity, normalized HVAC rating, battery state of charge, remaining charging
space, currently available discharge energy, current non-shiftable load, and
current heating demand. It is called capacity-aware because battery size and the
currently available charging and discharging space are supplied explicitly.
The first two variables describe the building's equipment, while the other five
describe its current operating condition. The remaining charging space is
calculated from capacity and the unused part of the battery, and the available
discharge energy is calculated from capacity and the current state of charge.

These inputs pass through a small two-layer neural network. For building $i$ at
time $t$, it produces one score for each candidate actor. The scores are converted
into probabilities,
\begin{equation}
...
During training, an actor is selected according to $g_{it}$, and that actor
produces the building action. CityLearn then executes the actions of all
buildings and returns the same reward defined in
Equation~\eqref{eq:report2-reward}. Thus, the router does not use a separate
reward. The shared part of the reward measures how well the total portfolio load
follows the target, while the building-specific part measures indoor comfort.
Consequently, an actor choice is encouraged only when it helps load tracking
without creating excessive comfort violations.

After a sequence of actions, MAPPO compares the received reward with the value
predicted before those actions. In simple terms, it estimates whether the result
was better or worse than expected. This comparison is then used to update the
probability of the selected actions. The probability calculation contains both
the router probabilities and the actor outputs. Therefore, when the result is
better than expected, training can increase the probability of similar
router--actor choices; when it is worse, their probability can be reduced. The
reward does not need to pass directly through the CityLearn simulator. It is
used to weight the action probability, and the resulting training error is
passed backwards through the actor-selection calculation to the router network.

Which parameters receive this update depends on the training stage. When the
pretrained actors are frozen, only the router parameters are updated, so the
router learns to use reliable existing actors without changing them. During
joint fine-tuning, the gradient can also update the allowed actor parameters. In
the most stable experiments, the encoders and TarMAC communication module remain
fixed and only the small action-output layers are allowed to change with a low
learning rate. A separate balance term also prevents all buildings from choosing
the same actor by keeping the average actor usage reasonably close to the
original group proportions. This term guides actor usage but is not an additional
CityLearn reward.

Soft Router can be trained effectively because the candidate actors already know
useful control behaviours, the original grouping provides a safe starting point,
and $\alpha$ introduces dynamic choice gradually. The probability-based selection
also allows the training error to reach the router, while the balance term and
staged freezing reduce the risk that one actor dominates or that useful
pretrained behaviour is lost. During deterministic testing, the actor with the
largest probability is selected directly for each building.
...
每个已经训练好的分组 actor 都被看作一个候选 actor，后面的训练说明也把它称为 expert。
在每个时间步，每栋建筑都会提供一组观测。这里的“观测”是控制器在当前时刻能够看到的信息，
例如电池 SOC、当前不可转移负荷、供暖需求、室内温度和时间信息，而不是整套建筑数据。

actor 网络的前半部分会把这些观测值转换成内部表示。内部表示不是新的物理测量值，而是网络
整理出的一组固定长度的数值，用来概括观测中对控制有用的信息。TarMAC 再把这组信息与其他
建筑的信息结合。普通 Soft Router 主要根据这组通信后的信息选择 actor。

但是，这组通信后的信息不一定能清楚保留与 actor 选择直接相关的电池能力和当前负荷等
物理信息。

capacity-aware 是 Soft Router 的增强版本。它会把明确的建筑物理信息额外提供给选择器，而
不是让选择器只从上面的内部表示中自己判断。它额外接收七个变量：归一化电池容量、归一化
HVAC 额定功率、电池 SOC、剩余可充电空间、当前可放电电量、当前不可转移负荷和当前供暖
需求。这个版本称为 capacity-aware，是因为它明确使用了电池容量以及当前可充电和可放电的
空间。前两个变量描述建筑的设备能力，后五个变量描述当前运行情况。剩余可充电空间由电池
容量和未使用的电量比例计算，当前可放电电量则由电池容量和当前 SOC 计算。

这些输入进入一个小型两层神经网络。对于时刻 $t$ 的建筑 $i$，网络会为每个候选 actor 输出
一个分数，再把这些分数转换成概率：
\begin{equation}
d_{it}=\operatorname{softmax}(z_{it}/\tau).
\end{equation}
...
训练时，实验根据 $g_{it}$ 的概率选择 actor，再由该 actor 生成建筑动作。CityLearn 执行
所有建筑的动作后，返回公式~\eqref{eq:report2-reward} 中定义的同一个 reward，因此 router
没有单独的 reward。reward 的共享部分衡量全部建筑的总负荷对目标曲线的跟踪情况，每栋建筑
自己的部分衡量室内舒适度。因此，只有当某次 actor 选择有助于负荷跟踪，并且没有造成过多
舒适度超限时，这个选择才会受到鼓励。

完成一段动作后，MAPPO 会比较实际得到的 reward 与执行动作前预测的价值。简单来说，就是
判断这次结果比预期更好还是更差，然后据此调整所选动作的概率。这个概率同时包含 router 的
选择概率和 actor 的动作输出。因此，如果结果比预期好，训练就会提高类似 router--actor
选择的概率；如果结果较差，就会降低其概率。reward 不需要直接穿过 CityLearn 模拟器，
而是用于调整动作概率对应的训练误差；这个误差再沿着 actor 选择过程反向传到 router 网络。

哪些参数会收到这个更新取决于训练阶段。冻结预训练 actor 时，只有 router 参数更新，因此
router 可以在不改变已有 actor 的情况下学习如何使用它们。联合微调时，更新也可以传到允许
训练的 actor 参数。在最稳定的实验中，encoder 和 TarMAC 通信模块保持不变，只让较小的
动作输出层以低学习率调整。实验还加入单独的平衡项，避免所有建筑都选择同一个 actor，方法
是让各 actor 的平均使用比例不要过度偏离原分组比例。这个平衡项只帮助控制 actor 的使用，
不是额外的 CityLearn reward。

Soft Router 能够有效训练，是因为候选 actor 已经掌握了有用的控制方式，原固定分组提供了
安全的训练起点，而 $\alpha$ 会逐步加入动态选择。基于概率的选择使训练误差能够传到 router，
平衡项和分阶段冻结则降低了单个 actor 被过度使用以及已有控制能力被破坏的风险。在确定性
测试时，每栋建筑直接选择概率最大的 actor。
\end{zhtranslation}
...
\section{Completed Experimental Framework}

The completed experimental framework supports selectable clustering, arbitrary feature
...
seed experiments. Each experiment records its configuration,
...
\begin{zhtranslation}
目前完成的实验框架支持可选聚类方法、任意特征列、cluster summary、hybrid fusion 消融、固定与
动态路由、capacity-aware router 输入、router entropy/balance 项、前馈与 GRU router、
基于 checkpoint 的阶段训练、分阶段冻结以及多随机种子实验。每次实验都会记录运行
