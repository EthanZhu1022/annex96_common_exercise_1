...
provides an observation $o_{it}$. This is the information visible to the
controller at that moment, such as battery state of charge (SOC), non-shiftable
load, heating demand, indoor temperature, and time information, rather than the
complete building dataset.

For the full-expert experiment, the router input has two complementary parts.
...
在每个时间步，每栋建筑提供观测 $o_{it}$。它是控制器在当前时刻能够看到的信息，例如电池
SOC、当前不可转移负荷、供暖需求、室内温度和时间信息，而不是整套建筑数据。

在 full-expert 实验中，router 输入由两个互补部分组成。第一部分由每栋建筑原固定分组的 actor
encoder 把观测转成 256 维隐藏特征，再由 TarMAC 在全部25栋建筑之间通信。建筑 $i$ 得到的
通信特征可以写成
\begin{equation}
h_{it}=\operatorname{TarMAC}_i\!\left(
\left\{\operatorname{Enc}_{c_j}(o_{jt})\right\}_{j=1}^{N}
