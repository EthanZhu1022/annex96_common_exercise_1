    ...
    **kwargs : Any
    ...
    def _get_observations_data(self) -> Mapping[str, Union[float, int]]:
        return {
            ...
            **{
            ...
            'district_net': getattr(self, '_Building__env', None).district_net if hasattr(self, '_Building__env') and self._Building__env is not None else 0.0,
            'district_tracking_error': getattr(self, '_Building__env', None).district_tracking_error if hasattr(self, '_Building__env') and self._Building__env is not None else 0.0,
        }
    ...
    def estimate_observation_space_limits(self, include_all: bool = None, periodic_normalization: bool = None) -> Tuple[
        ...
        for key in observation_names:
            ...
            elif key == 'district_net':
                # District net is sum of positive net consumption across all buildings
                low_limit[key] = 0.0
                high_limit[key] = 2500.0  # Rough estimate for 25 buildings
            
            elif key == 'district_tracking_error':
                # Error can be positive or negative, use district_target range
                district_target_data = self.district_load_target.district_load_target[
                    self.episode_tracker.simulation_start_time_step:self.episode_tracker.simulation_end_time_step + 1
                ]
                max_target = max(district_target_data) if len(district_target_data) > 0 else 100.0
                low_limit[key] = -max_target * 2.0
                high_limit[key] = max_target * 2.0
