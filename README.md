...
The `mappo/` directory contains a Hierarchical MAPPO with Communication baseline.

### Train-test workflow

The script trains on one month, then automatically evaluates on the next month (no
parameter updates during evaluation). Default windows follow the README instructions:

| Climate | Train    | Test      | Train steps  | Test steps   |
|---------|----------|-----------|-------------|--------------|
| VT      | January  | February  | 0 – 743     | 744 – 1415   |
| TX      | August   | September | 5088 – 5831 | 5832 – 6551  |

Time windows are set via code/config — **do not edit schema.json**.
...
|---|---|---|
| `--climate` | `VT` | Dataset: `VT` or `TX` |
| `--n_episodes` | `100` | Training episodes |
| `--train_month` | `1` (VT) / `8` (TX) | Month index 1–12 for training |
| `--test_month` | `2` (VT) / `9` (TX) | Month index 1–12 for evaluation |
| `--save_every` | `10` | Checkpoint interval (episodes) |
| `--eval_freq` | — | Backward-compat alias for `--save_every` |
| `--backup_dir` | auto | Custom backup directory |
| `--no_test` | — | Skip test evaluation |
| `--no_comm` | — | Disable agent communication |
| `--seed` | `42` | Random seed |
| `--save_dir` | `results/mappo` | Output directory |
...
```
results/mappo_vt/
  actor_0.pt … actor_4.pt    # cluster policy networks
  critic.pt                  # centralized value function
  comm_net_0.pt … comm_net_4.pt  # communication encoders (if comm enabled)
  run_config.json            # full hyperparameters + time-window settings
  latest_metrics.json        # train + test summary
  training_curves.png        # reward / loss / KPI plots
  test_metrics.json          # test-episode KPIs
  test_metrics.csv           # same, tabular
  backups/
    20260408_120000_VT_seed42/
      actor_0.pt … (full copy of checkpoint)
```

### Reproducibility

Two runs with the same `--seed` produce close (not bitwise-identical on GPU) summary
metrics. The script prints a reproducibility block at startup:
