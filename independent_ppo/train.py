...
def train(cfg: Config) -> PPO:
    ...
    for iteration in range(1, cfg.n_iterations + 1):
        ...
        # Extract KPIs from custom_metrics logged by CE1Callback
        cm   = result.get("custom_metrics", {})
        kpis = {
        ...
        if iteration % 10 == 0:
            ...
            ramp_val       = kpis.get("kpi/ramping_average", float("nan"))
            ramp_str       = f"{ramp_val:.3f}" if isinstance(ramp_val, float) and np.isfinite(ramp_val) else "nan"
            print(
                f"[train] Iter {iteration:4d} | ep_rew_mean {ep_reward_mean:9.2f} | "
                f"portfolio_sum {portfolio_mean:9.2f} | ramp {ramp_str}"
            )
