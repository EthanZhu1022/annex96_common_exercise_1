"""
Grouped MAPPO with DIAL-style global communication and grouped actor heads.

Actor path:
  obs(group_k) -> encoder_k -> DIAL-style global communication -> action_head_k
"""
...
from mappo_grouped_dial.cluster import run_clustering  # noqa: E402
from mappo_grouped_dial.dial_comm import DIALCommunicationModule  # noqa: E402
from mappo_grouped_dial.env import CityLearnMAPPOEnv  # noqa: E402
...
class Config:
    ...
    wandb_name: str = "mappo-grouped-dial"
    save_dir: str = "results/mappo_grouped_dial"
    backup_dir: Optional[str] = None

    comm_method: str = "dial"
    comm_hidden_dim: int = 64
    comm_rounds: int = 1
    comm_use_residual: bool = True
    comm_dropout: float = 0.0
    comm_scope: str = "global"
    comm_message_dim: int = 32
    comm_dru_noise: float = 0.5
    comm_discretize_eval: bool = True
...
def _build_grouped_components(
    ...
    global_comm = DIALCommunicationModule(
        hidden_dim=cfg.hidden_size,
        comm_hidden_dim=cfg.comm_hidden_dim,
        message_dim=cfg.comm_message_dim,
        comm_rounds=cfg.comm_rounds,
        use_residual=cfg.comm_use_residual,
        dropout=cfg.comm_dropout,
        dru_noise=cfg.comm_dru_noise,
        discretize_eval=cfg.comm_discretize_eval,
    ).to(device)
...
def evaluate_on_test(
    ...
    print("\n" + "=" * 65)
    print(
        f"TEST EVALUATION (grouped DIAL) | {cfg.climate} | {month_name} "
...
def train(cfg: Config) -> None:
    ...
    print("\nGrouped MAPPO DIAL components built:")
    ...
    print("\nGrouped MAPPO DIAL training complete.")
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(
        description="Grouped MAPPO with DIAL-style global communication and grouped actor heads."
    )
    ...
    parser.add_argument("--wandb_name", default="mappo-grouped-dial")
    parser.add_argument("--save_dir", default="results/mappo_grouped_dial")
    ...
    parser.add_argument("--comm_message_dim", type=int, default=32)
    parser.add_argument("--comm_dru_noise", type=float, default=0.5)
    ...
    parser.add_argument("--no_comm_discretize_eval", action="store_true", default=False)
    ...
    return Config(
        ...
        comm_message_dim=args.comm_message_dim,
        comm_dru_noise=args.comm_dru_noise,
        comm_discretize_eval=not args.no_comm_discretize_eval,
    )
